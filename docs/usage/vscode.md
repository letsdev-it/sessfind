...
- The `sessfind` binary. If VS Code is launched from the desktop and does not
  inherit your shell `PATH`, the extension automatically detects Cargo's
  standard installation path (`~/.cargo/bin/sessfind`). Set
